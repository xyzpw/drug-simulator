...
def initiate(drugInfo: object, pkInfo: object):
    ...
    if pkInfo.usingTimeOrElapse:
        ...
        if isBiphasic:
            if tmaxedEpoch < drugInfo.distTime + tmaxedEpoch:
                phase = "distribution"
...
def initiateDR(drugInfo: object, pkInfo: object):
    ...
    while True:
        ...
        if massUnit != None and phase == "elimination" and delayedPhase == "elimination":
            adjustedConcentration, massUnit, adjustedPrecision = adjustConcentrationFromUnit(
                totalConcentration,
            ...
            if pkInfo.dr_max:
                concentration_response = defaultDrResult(
                    phase, delayedPhase,
                    adjustedConcentration, adjustedPrecision,
                    massUnit, maxMassUnit,
                    drMaxConcentration
                )
            else:
                concentration_response = defaultDrResult(
